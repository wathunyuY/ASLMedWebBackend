...
//	private String root_dir = ConfigMapHelper.getConfigValue("CONFIG_CONSTANTS.WINDOW_IMAGE_ROOT_DIRECTORY");
	...
	public boolean deletePictures(String rootPath, String imageName) {
		String path = this.root_dir+File.separator+ rootPath +File.separator+imageName;
		try {	
			Files.delete(Paths.get(path+"_Original.png"));
			List<String> size = ConfigMapHelper.getConfigValueList("CONFIG_CONSTANTS.PROFILE_PIC_SIZE");
			for (String s : size) {
				Files.delete(Paths.get(path+"_"+s+".png"));
			}
	...
	public boolean deletePictures(String fullPath) {
		String path = this.root_dir+File.separator+fullPath;
		try {	
			Files.delete(Paths.get(path+"_Original.png"));
			List<String> size = ConfigMapHelper.getConfigValueList("CONFIG_CONSTANTS.PROFILE_PIC_SIZE");
			for (String s : size) {
	...
	public void convertBase64ToFile(String sourceData, String filePath, String imageName) throws IOException {
		// String sourceData =
		...
		try {
			List<String> strList = ConfigMapHelper.getConfigValueList("CONFIG_CONSTANTS.PROFILE_PIC_SIZE");
			List<Integer> sizeList = strList.stream().map(Integer::parseInt).collect(Collectors.toList());

			String base64Image = sourceData.split(",")[1];
			byte[] imageBytes = DatatypeConverter.parseBase64Binary(base64Image);
			BufferedImage img = ImageIO.read(new ByteArrayInputStream(imageBytes));
			File outputfile = new File(this.root_dir + File.separator + filePath + File.separator + imageName + "_Original.png");
			logger.debug("path pic = "+this.root_dir + File.separator + filePath + File.separator + imageName + "_Original.png");
			ImageIO.write(img, "png", outputfile);
			for (int i = 0; i < sizeList.size(); i++) {
				int size = sizeList.get(i);
				img = getScaledImage(img, size, size);
				outputfile = new File(this.root_dir + File.separator + filePath + File.separator + imageName + "_" + size + ".png");
				ImageIO.write(img, "png", outputfile);
			}
		}catch (Exception e) {
